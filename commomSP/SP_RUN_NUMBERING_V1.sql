...
BEGIN
    ...
    DECLARE @breakPattern NVARCHAR(50) = '';
    ...
    /* ===============================
       detect break pattern with SEPARATORS
    =============================== */
    IF CHARINDEX('yyyy/mm/dd', @formatId) > 0 BEGIN SET @breakPattern = 'yyyy/mm/dd'; SET @separator = '/'; END
    ELSE IF CHARINDEX('yyyy\mm\dd', @formatId) > 0 BEGIN SET @breakPattern = 'yyyy\mm\dd'; SET @separator = '\'; END
    ELSE IF CHARINDEX('yyyy-mm-dd', @formatId) > 0 BEGIN SET @breakPattern = 'yyyy-mm-dd'; SET @separator = '-'; END
    
    ELSE IF CHARINDEX('yyyy/mm', @formatId) > 0 BEGIN SET @breakPattern = 'yyyy/mm'; SET @separator = '/'; END
    ELSE IF CHARINDEX('yyyy\mm', @formatId) > 0 BEGIN SET @breakPattern = 'yyyy\mm'; SET @separator = '\'; END
    ELSE IF CHARINDEX('yyyy-mm', @formatId) > 0 BEGIN SET @breakPattern = 'yyyy-mm'; SET @separator = '-'; END
    
    ELSE IF CHARINDEX('yy/mm/dd', @formatId) > 0 BEGIN SET @breakPattern = 'yy/mm/dd'; SET @separator = '/'; END
    ELSE IF CHARINDEX('yy\mm\dd', @formatId) > 0 BEGIN SET @breakPattern = 'yy\mm\dd'; SET @separator = '\'; END
    ELSE IF CHARINDEX('yy-mm-dd', @formatId) > 0 BEGIN SET @breakPattern = 'yy-mm-dd'; SET @separator = '-'; END
    
    ELSE IF CHARINDEX('yy/mm', @formatId) > 0 BEGIN SET @breakPattern = 'yy/mm'; SET @separator = '/'; END
    ELSE IF CHARINDEX('yy\mm', @formatId) > 0 BEGIN SET @breakPattern = 'yy\mm'; SET @separator = '\'; END
    ELSE IF CHARINDEX('yy-mm', @formatId) > 0 BEGIN SET @breakPattern = 'yy-mm'; SET @separator = '-'; END
    
    ELSE IF CHARINDEX('mm/dd', @formatId) > 0 BEGIN SET @breakPattern = 'mm/dd'; SET @separator = '/'; END
    ELSE IF CHARINDEX('mm\dd', @formatId) > 0 BEGIN SET @breakPattern = 'mm\dd'; SET @separator = '\'; END
    ELSE IF CHARINDEX('mm-dd', @formatId) > 0 BEGIN SET @breakPattern = 'mm-dd'; SET @separator = '-'; END

    ELSE IF CHARINDEX(':hhmmss', @formatId) > 0 SET @breakPattern = ':hhmmss'
    ...
    /* ===============================
       build break key
    =============================== */
    IF @breakPattern = 'yyyy/mm/dd' SET @breakKey = FORMAT(@now, 'yyyy') + '/' + FORMAT(@now, 'MM') + '/' + FORMAT(@now, 'dd');
    ELSE IF @breakPattern = 'yyyy\mm\dd' SET @breakKey = FORMAT(@now, 'yyyy') + '\' + FORMAT(@now, 'MM') + '\' + FORMAT(@now, 'dd');
    ELSE IF @breakPattern = 'yyyy-mm-dd' SET @breakKey = FORMAT(@now, 'yyyy') + '-' + FORMAT(@now, 'MM') + '-' + FORMAT(@now, 'dd');
    
    ELSE IF @breakPattern = 'yyyy/mm' SET @breakKey = FORMAT(@now, 'yyyy') + '/' + FORMAT(@now, 'MM');
    ELSE IF @breakPattern = 'yyyy\mm' SET @breakKey = FORMAT(@now, 'yyyy') + '\' + FORMAT(@now, 'MM');
    ELSE IF @breakPattern = 'yyyy-mm' SET @breakKey = FORMAT(@now, 'yyyy') + '-' + FORMAT(@now, 'MM');
    
    ELSE IF @breakPattern = 'yy/mm/dd' SET @breakKey = FORMAT(@now, 'yy') + '/' + FORMAT(@now, 'MM') + '/' + FORMAT(@now, 'dd');
    ELSE IF @breakPattern = 'yy\mm\dd' SET @breakKey = FORMAT(@now, 'yy') + '\' + FORMAT(@now, 'MM') + '\' + FORMAT(@now, 'dd');
    ELSE IF @breakPattern = 'yy-mm-dd' SET @breakKey = FORMAT(@now, 'yy') + '-' + FORMAT(@now, 'MM') + '-' + FORMAT(@now, 'dd');
    
    ELSE IF @breakPattern = 'yy/mm' SET @breakKey = FORMAT(@now, 'yy') + '/' + FORMAT(@now, 'MM');
    ELSE IF @breakPattern = 'yy\mm' SET @breakKey = FORMAT(@now, 'yy') + '\' + FORMAT(@now, 'MM');
    ELSE IF @breakPattern = 'yy-mm' SET @breakKey = FORMAT(@now, 'yy') + '-' + FORMAT(@now, 'MM');
    
    ELSE IF @breakPattern = 'mm/dd' SET @breakKey = FORMAT(@now, 'MM') + '/' + FORMAT(@now, 'dd');
    ELSE IF @breakPattern = 'mm\dd' SET @breakKey = FORMAT(@now, 'MM') + '\' + FORMAT(@now, 'dd');
    ELSE IF @breakPattern = 'mm-dd' SET @breakKey = FORMAT(@now, 'MM') + '-' + FORMAT(@now, 'dd');

    ELSE IF @breakPattern = ':hhmmss' SET @breakKey = ':' + FORMAT(@now, 'HHmmss');
    ...
    IF @breakPattern <> ''
        SET @GeneratedNo = REPLACE(@GeneratedNo, @breakPattern, @breakKey);
