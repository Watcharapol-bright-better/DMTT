Private Sub Workbook_Open()
    ...
    ' ===============================
    ' Check Data Header A2
    ' ===============================
    If Trim(Worksheets(dataWS).Cells(2, "A").Value) = "%1_ROW_NO_TBL%" Then
        Exit Sub
    End If
    ...
    For index = 2 To lastRow
        ...
        If index = lastRow Then
            ...
            ' insert 2 blank rows
            Worksheets(mainWS).Rows(rowStart + 1).Insert
            Worksheets(mainWS).Rows(rowStart + 1).Insert
        
            ' clear format so it becomes true empty rows
            Worksheets(mainWS).Rows(rowStart + 1 & ":" & rowStart + 2).ClearFormats
        
            ' delete config row
            Worksheets(mainWS).Rows(14).Delete Shift:=xlUp
