...
function onCloseMassageDialog() {
  ...
  if (errorStatus.value == "0" && _event.value == "CLOSED_MESSAGE_DIALOG") {
    ...
    let isCreated = liElement.textContent.includes("Successfully Submitted.(CMN_N0012)");
    console.log(isCreated);

    if (textColor == "rgb(0, 0, 255)" && isCreated) {
...
function defaultRate() {
  let currency = document.getElementById("TLN_1_I_CURRENCY").value;
  if (currency === "THB") {
    document.getElementById("TLN_1_I_EXG_RATE").value = 1;
  }
}
