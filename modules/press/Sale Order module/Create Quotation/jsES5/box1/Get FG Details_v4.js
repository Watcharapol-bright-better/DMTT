var data = TALON.getTargetData();


var csCode = data['I_CSCODE']; // Customer Code
var qtPattern = data['I_TYPE']; // Pattern

// TALON.addMsg(csCode);
// TALON.addMsg(qtPattern);


if (!csCode || !qtPattern) {

    if (!csCode && !qtPattern) {
      TALON.addErrorMsg("⚠️ Customer Code and Pattern are required fields.");
      TALON.setIsSuccess(false);
    } else if (!csCode) {
      TALON.addErrorMsg("⚠️ Customer Code is a required field.");
      TALON.setIsSuccess(false);
    } else if (!qtPattern) {
      TALON.addErrorMsg("⚠️ Pattern is a required field.");
      TALON.setIsSuccess(false);
    }

} else {

  var qtID = RunningNo.genId('DMTT_N_QT', 'Qyyyymmxx', false);

    // INSERT TO : T_PR_QT table
    // I_QT_STATUS : '0'
  var sql =
      "" +
      "SELECT " +
      "     [MAIN].[I_QT_NO] " +
      "    ,[MAIN].[I_QTPATTERN] " +
      "    ,[MAIN].[I_CSCODE] " +
      "    ,[MAIN].[I_QT_STATUS] " +
      "    ,[MAIN].[I_ITEMCODE] " +
      "    ,[MAIN].[I_DESC] " +
      "    ,[MAIN].[I_COMMODITY] " +
      "    ,[MAIN].[I_THICK] " +
      "    ,[MAIN].[I_WIDTH] " +
      "    ,[MAIN].[I_PROD_WGT] " +
      "    ,[MAIN].[I_RM_WGT] " +
      "    ,[MAIN].[I_LOSS_WGT] " +
      "    ,[MAIN].[I_PITCH] " +
      "    ,[MAIN].[MAT_COST] " +
      "    ,[MAIN].[SCRAP_COST] " +
      "    ,[MAIN].[I_RM_AMT] " +          // Material amount
      "    ,[MAIN].[I_LOSS_AMT] " +        // Scrap amount
      "    ,[MAIN].[I_FEE_PROCESS] " +     // Press Processing
      "    ,[MAIN].[I_FEE_CUSTOM] " +
      "    ,[MAIN].[I_FEE_PACK] " +        // Package Material
      "    ,[MAIN].[I_FEE_EXPENSE] " +     // Maintenance Expense
      "    ,[MAIN].[I_FEE_DLY] " +         // Delivery Fee
      "    ,[MAIN].[I_FEE_MGM] " +
      "    ,ROUND( " +
      "          ISNULL([MAIN].[I_FEE_PROCESS], 0) " +
      "        + ISNULL([MAIN].[I_FEE_EXPENSE], 0) " +
      "        + ISNULL([MAIN].[I_FEE_CUSTOM], 0) " +
      "        + ISNULL([MAIN].[I_FEE_PACK], 0) " +
      "        + ISNULL([MAIN].[I_FEE_DLY], 0) " +
      "      , 2) AS [MANAGEMENT_EXPENSE] " +
      "    ,ROUND( " +
      "          ISNULL([MAIN].[I_RM_AMT], 0) " +
      "        + ISNULL([MAIN].[I_LOSS_AMT], 0) " +
      "        + ISNULL([MAIN].[I_FEE_PROCESS], 0) " +
      "        + ISNULL([MAIN].[I_FEE_CUSTOM], 0) " +
      "        + ISNULL([MAIN].[I_FEE_PACK], 0) " +
      "        + ISNULL([MAIN].[I_FEE_EXPENSE], 0) " +
      "        + ISNULL([MAIN].[I_FEE_DLY], 0) " +
      "        + ISNULL([MAIN].[I_FEE_MGM], 0) " +
      "      , 2) AS [I_SELLING_PRICE] " +
      "FROM ( " +
      "    SELECT " +
      "         '"+qtID+"' AS [I_QT_NO] " +
      "        ,[Q].[I_QTPATTERN] " +
      "        ,[Q].[I_CSCODE] " +
      "        ,'0' AS [I_QT_STATUS] " +
      "        ,[Q].[I_ITEMCODE] " +
      "        ,[Q].[I_DESC] " +
      "        ,[Q].[I_COMMODITY] " +
      "        ,[Q].[I_THICK] " +
      "        ,[Q].[I_WIDTH] " +
      "        ,[Q].[I_PROD_WGT] " +
      "        ,[Q].[I_RM_WGT] " +
      "        ,[Q].[I_LOSS_WGT] " +
      "        ,[Q].[I_PITCH] " +
      "        ,[Q].[MAT_COST] " +
      "        ,[Q].[SCRAP_COST] " +
      "        ,([Q].[MAT_COST] * [Q].[I_RM_WGT]) AS [I_RM_AMT] " +
      "        ,([Q].[SCRAP_COST] * [Q].[I_LOSS_WGT]) AS [I_LOSS_AMT] " +
      "        ,[Q].[I_FEE_PROCESS] " +
      "        ,[Q].[I_FEE_CUSTOM] " +
      "        ,[Q].[I_FEE_PACK] " +
      "        ,[Q].[I_FEE_EXPENSE] " +
      "        ,[Q].[I_FEE_DLY] " +
      "        ,[Q].[I_FEE_MGM] " +
      "    FROM ( " +
      "        SELECT " +
      "             [MP].[I_QTPATTERN] " +
      "            ,[MP].[I_CSCODE] " +
      "            ,[MP].[I_ITEMCODE] " +
      "            ,[MP].[I_DESC] " +
      "            ,[MP].[I_COMMODITY] " +
      "            ,[MP].[I_THICK] " +
      "            ,[MP].[I_WIDTH] " +
      "            ,([MP].[I_PROD_WGT] / 100.0) * (200.0 - [MP].[I_YIELD]) AS [I_PROD_WGT] " +
      "            ,([MP].[I_RM_WGT] / 100.0) * (200.0 - [MP].[I_YIELD]) AS [I_RM_WGT] " +
      "            ,([MP].[I_LOSS_WGT] / 100.0) * (200.0 - [MP].[I_YIELD]) AS [I_LOSS_WGT] " +
      "            ,[MP].[I_PITCH] " +
      "            ,ISNULL(NULL, 422.49) AS [MAT_COST] " +
      "            ,ISNULL(NULL, -321.84) AS [SCRAP_COST] " +
      "            ,[MP].[I_FEE_PROCESS] " +
      "            ,[MP].[I_FEE_CUSTOM] " +
      "            ,[MP].[I_FEE_PACK] " +
      "            ,[MP].[I_FEE_EXPENSE] " +
      "            ,[MP].[I_FEE_DLY] " +
      "            ,COALESCE([MP].[I_FEE_PROCESS], 0) " +
      "             + COALESCE([MP].[I_FEE_CUSTOM], 0) " +
      "             + COALESCE([MP].[I_FEE_PACK], 0) " +
      "             + COALESCE([MP].[I_FEE_EXPENSE], 0) " +
      "             + COALESCE([MP].[I_FEE_DLY], 0) AS [I_FEE_MGM] " +
      "        FROM [MS_PRFG] AS [MP] " +
      "WHERE [MP].[I_CSCODE] = '" + csCode + "' " +
      "  AND [MP].[I_QTPATTERN] = '" + qtPattern + "' " +
      "    ) AS [Q] " +
      ") AS [MAIN]";

    // "WHERE [MP].[I_CSCOD] = 'C006' " +
    // "  AND [MP].[I_QTPATTERN] = '1' ";

    // TALON.addMsg(sql);
  var data = TalonDbUtil.select(TALON.getDbConfig(), sql);
  
  
  if (data.length > 0) {
    TALON.setSearchedDisplayList(2, data);
    TALON.setSearchConditionData('I_QT_NO', qtID, '')
  } else {
    TALON.setIsSuccess(false);
    TALON.addErrorMsg("⚠️ Material price not found");
  }  

}

