...
function onClickSearch() {
  ...
  document.getElementById("BUTTON_MDL_R:0:POS_SEARCH_BTN").click();
}

function checkAndClearOnError(targetElement) {
  if (!targetElement) return;
  setTimeout(() => {
    const errorMessage = document.querySelector('ul li.tln-message.message-error.message-error-icon');
    
    if (errorMessage) {
      console.log("Error detected:", errorMessage.textContent);
      
      // ล้างข้อมูลในฟิลด์ที่มี cursor
      if (targetElement) {
        targetElement.value = "";
        console.log(`Cleared field: ${targetElement.id}`);
      }
      
      // ล้าง error message (optional)
      // errorMessage.remove();
      
      // Focus กลับไปที่ฟิลด์นั้น
...
function bindScanEvents() {
  ...
  watchInputFilled(ship_id, () => {
    focusNext(ship_mask);
    onClickSearch();
    checkAndClearOnError(ship_id); 
    scanningBox();
  });

  watchInputFilled(ship_mask, () => {
    focusNext(sample_tag);
    checkAndClearOnError(ship_mask); 
    scanningBox();
  });

  watchInputFilled(sample_tag, () => {
    focusNext(pallet_tag);
    checkAndClearOnError(sample_tag); 
    scanningBox();
  });

  watchInputFilled(pallet_tag, () => {
    checkAndClearOnError(pallet_tag); 
    scanningBox();
  });
