(function () {
  ...
  const RESULT_MSG_KEY =
    "Redirect to Create Work Order Plan Screen , please wait ....";
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      if (
        e.target &&
        (e.target.id === BTN_CONFIRM_SRC ||
          e.target.closest("#" + BTN_CONFIRM_SRC))
      ) {
  ...
  const ID_DATE = "CNDTN_STD:0:_TEXT",
    ID_PART = "CNDTN_STD:1:_TEXT",
    ID_QTY = "CNDTN_STD:2:_TEXT";
  const BLOCK_DATE = "TLN_1_I_DATE",
    BLOCK_PART = "TLN_1_I_ITEMCODE",
    BLOCK_QTY = "TLN_1_TARGET_QTY";
  const GANTT_IDS = {
    y: "tln_gantt_1_block_target_year",
    m: "tln_gantt_1_block_target_month",
    d: "tln_gantt_1_block_target_day",
  };
  ...
  function initAutoRedirectObserver() {
    ...
    const observer = new MutationObserver(function () {
      ...
      for (let j = 0; j < msgs.length; j++) {
        if (
          msgs[j].textContent.indexOf(RESULT_MSG_KEY) !== -1 &&
          !window._is_waiting
        ) {
  ...
  function safeUpdateInput(id, val) {
    ...
    if (el.value != String(val)) {
      el.value = val || "";
      if (window.TALON && TALON.putBindValue)
        TALON.putBindValue("FREE", id, el.value);
  ...
  setTimeout(function () {
    ...
    if (saved) {
      ...
      if (d.pickedDate) {
        document.getElementById(ID_DATE).value = d.pickedDate;
        processDateSync(d.pickedDate);
      }
      if (d.partId) {
        document.getElementById(ID_PART).value = d.partId;
        safeUpdateInput(BLOCK_PART, d.partId);
      }
      if (d.targetQty) {
        document.getElementById(ID_QTY).value = d.targetQty;
        safeUpdateInput(BLOCK_QTY, d.targetQty);
      }
    }
  }, 1000);
