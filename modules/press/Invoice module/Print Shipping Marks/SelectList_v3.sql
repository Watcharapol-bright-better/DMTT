SELECT
     [GROUP_NO]
    ,ROW_NUMBER() OVER(PARTITION BY [I_SHIP_INST], [GROUP_NO] ORDER BY [INTERNAL_NO]) AS [LINE_NO] -- Line No
    ,[I_SHIP_INST]
    ,[QRCODE]
    ,[I_SONO]
    ,[I_CSCODE]
...
FROM (
    SELECT
         CEILING(ROW_NUMBER() OVER(PARTITION BY [SD].[I_SHIP_INST] ORDER BY [SD].[INTERNAL_NO]) / 9.0) AS [GROUP_NO] -- Group Number
    ...
    WHERE [SD].[I_SHIP_INST] = /**%I_SHIP_INST%**/'SI2601150006'

) AS [M]

--ORDER BY [INTERNAL_NO]
