...
if (shipOrderNo !== '' || shipOrderNo !== null) {
    ...
    var sqlBox2 = ""
        ...
        + "LEFT JOIN [MS_PRFG] AS [CS] "
        + "    ON [CS].[I_ITEMCODE] = [D].[I_ITEMCODE] "
        + "LEFT JOIN [T_PR_SORD_D] AS [SOD] "
        + "    ON [SOD].[I_SONO] = [D].[I_SONO] "
        + "   AND [SOD].[I_ITEMCODE] = [D].[I_ITEMCODE] "
        + "LEFT JOIN [T_PR_STOCK] AS [SK] "
        + "    ON [SK].[I_ITEMCODE] = [D].[I_ITEMCODE] "
        + "LEFT JOIN [T_PR_WOH] AS [WO] "
        + "    ON [WO].[I_SONO] = [D].[I_SONO] "
        + "   AND [WO].[I_ITEMCODE] = [D].[I_ITEMCODE] "
