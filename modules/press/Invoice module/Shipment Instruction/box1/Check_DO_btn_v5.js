...
var sql =
    ...
    "    ,[MAIN].[I_DESC] " +
    "    ,[MAIN].[I_BOX_PALLET] " +
    "	,CEILING([MAIN].[I_BALANCE_QTY]/ ([MAIN].[I_PCS_BOX] * [MAIN].[I_BOX_PALLET])) AS [I_PALLET_QTY] " +
    "    ,[MAIN].[I_SHIP_QTY] " +
    "    ,[MAIN].[I_BALANCE_QTY] " +
    "    ,[MAIN].[I_BOX_QTY] " +
    "    ,[MAIN].[I_LOTNO_FR] " +
    ...
    "            [STK].[STK_BALANCE_QTY], " +
    "            IIF( " +
    "                ISNULL([SD].[I_QTY], 0) = ISNULL([SHIP_SUM].[TOTAL_SHIP_QTY], 0), " +
    "                ISNULL([SHIP_SUM].[TOTAL_SHIP_QTY], 0), " +
    "                ISNULL([SD].[I_QTY], 0) - ISNULL([SHIP_SUM].[TOTAL_SHIP_QTY], 0) " +
    "            ) " +
    "        ) AS [I_BALANCE_QTY] " +
    "		,CEILING( " +
    "			CAST(ISNULL([STK].[STK_BALANCE_QTY],ISNULL([SD].[I_QTY], 0)) AS DECIMAL(18,4)) " +
    "			/ NULLIF([MP].[I_PCS_BOX], 0) " +
    "		) AS [I_BOX_QTY] " +
    ...
    "    WHERE [SD].[I_COMPCLS] = '00' " +
    "      AND [SD].[I_DLYDATE] = '" + fmtDate + "' " +
    ") AS [MAIN]" +
    "WHERE [MAIN].[I_BALANCE_QTY] > 0";
