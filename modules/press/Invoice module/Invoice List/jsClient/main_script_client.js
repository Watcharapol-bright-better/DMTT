document.addEventListener("DOMContentLoaded", function () {
  setTimeout(function () {
    ...
    if (status !== "") {
      ...
      getButtonElementByValue("Get Fech Token").click();
    }
  }, 700);
});
...
function authGetFechToken() {
  let btn = getButtonElementByValue("Receive OTP");
  if (btn) {
    btn.click();
  }
}
...
function resizeContents_end() {
  Array.from(document.getElementsByClassName("selectionTableRow")).forEach(
    (chk) => chk.addEventListener("change", onCheckboxClick),
  );
  ...
  getButtonElementByValue("Receive OTP").style.display = "none"; // Receive OTP
  getButtonElementByValue("Get Fech Token").style.display = "none"; // Get Fech Token

  const status = document.getElementsByName("CNDTN_DISPLAY")[0];
  const authorizeGA = getButtonElementByValue("Authorize GA"); // Authorize GA
  const interfaceGA = getButtonElementByValue("Interface to GA"); // Interface to GA
  const cancel = getButtonElementByValue("Cancel Interface"); // Cancel Interface
