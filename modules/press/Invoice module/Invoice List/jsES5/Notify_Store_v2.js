...
function getApprStatusBadge(statusCode) {
  ...
  return '<span style="background-color: ' + backgroundColor + "; color: " + textColor + '; padding: 4px 8px; border-radius: 4px; font-weight: 500;">' + statusText + "</span>";
}
...
function createTableHTML(items) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    html += '<td style="border: 1px solid #ddd; padding: 10px;"><strong>' + (item.I_INVOICE_NO || "") + "</strong></td>";
    html += '<td style="border: 1px solid #ddd; padding: 10px;"><span style="background-color: #e3f2fd; padding: 4px 8px; border-radius: 4px;">' + getInvoiceTypeName(item.I_TYPE) + "</span></td>";
    html += '<td style="border: 1px solid #ddd; padding: 10px;">' + (item.I_SHIP_ORDER_NO || "") + "</td>";
    html += '<td style="border: 1px solid #ddd; padding: 10px;">' + (item.I_NAME || "") + "</td>";
    html += '<td style="border: 1px solid #ddd; padding: 10px;">' + (item.I_SHIP_TO || "") + "</td>";
    html += '<td style="border: 1px solid #ddd; padding: 10px;">' + DateFmt.formatDate(item.I_INVOICE_DATE.toString()) + "</td>";
    html += '<td style="border: 1px solid #ddd; padding: 10px;">' + getApprStatusBadge(item.I_APPR_STATUS) + "</td>";
    html += "</tr>";
  }
...
if (!selectedItems || selectedItems.length === 0) {
  ...
  var MAIL_SEND_CC = ["watcharapol@bright-better.com", "apirak@bright-better.com", "aelysiam@gmail.com"];

  var MAIL_SEND_BCC = ["pawared@bright-better.com", "apirak@bright-better.com", "wutthiphat@bright-better.com"];
