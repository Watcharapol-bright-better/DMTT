 ...
 * 
 * @param {string} [skey]
 *        Optional tertiary field name (3rd key)
 *        (example: 'INTERNAL_NO').
 *        If provided, value format will be:
 ...
 * }
 *
 * // Focusout Scope (Group Level Example for 2 key use)
 * let checkbox = %2_CHK%;
 * if (checkbox) {
 ...
 *
 * 
 * // Focusout Scope (Detail Row Example for 3 key use)
 * let checkbox2 = %2_SEL_CHK%;
 * if (checkbox2) {
 *     checkbox2.addEventListener('focusout', function () {
 *         onCheckboxClick(this, 2, false, 'I_SHIP_INST', 'INTERNAL_NO', 'LINENO');
 *     });
 *
 *     checkbox2.dispatchEvent(new FocusEvent('focusout', {
 *         bubbles: true
...
const SelectChk = ( () => {
    ...
    function click(chk, block, isLvl, pkey, skey, tkey) {
        if (!chk.name) return;
      ...
      if (isLvl) {
        ...
        lineDetail(chk, i, pk, block, hold, skey, tkey);
      } else { 
        lineDetail(chk, i, pk, block, hold, skey, tkey);
      } 
    ...
    function lineDetail(chk, i, pk, block, hold, skey, tkey) {
        const sel = parse(hold.value);
        if (chk.checked && pk.value.trim()) {
            if (skey && tkey) {
                const key2 = document.querySelector(`#TLN_${block}_${skey}_${i}`);
                const key3 = document.querySelector(`#TLN_${block}_${tkey}_${i}`);
                sel[i] = key2 && key3 ? `${pk.value}|${key2.value}|${key3.value}` : pk.value;
            } else if (skey) {
                const key2 = document.querySelector(`#TLN_${block}_${skey}_${i}`);
                sel[i] = key2 ? `${pk.value}|${key2.value}` : pk.value;
            } else {
                sel[i] = pk.value;
            }
        } else {
